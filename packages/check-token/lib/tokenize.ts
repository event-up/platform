...
export const generateRegistrationToken = async (
  ...
  const signature = createHmac("sha256", env.CHECKIN_TOKEN_SECRET)
...
export const verifyRegistrationToken = async (
  ...
  const expectedSignature = createHmac("sha256", env.CHECKIN_TOKEN_SECRET)
    .update(payloadBase64)
    .digest("base64url");

  const isVaid = timingSafeEqual(
