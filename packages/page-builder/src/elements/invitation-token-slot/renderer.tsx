...
type InvitationTokenSlotRendererProps = WithPuckProps<
  EventPageComponents["InvitationTokenSlot"]
>;

export function InvitationTokenSlotRenderer({
  title,
  description,
  puck,
}: InvitationTokenSlotRendererProps) {
  const metadata = puck.metadata as EventPageMetadata | undefined;

  if (metadata?.invitationSlot) {
    return <>{metadata.invitationSlot}</>;
  }
