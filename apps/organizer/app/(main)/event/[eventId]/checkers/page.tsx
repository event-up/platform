...
import { Plus, Trash2, DoorOpen, UserCheck, Mail, User, TowerControl, Command } from "lucide-react";
...
export default function CheckersPage() {
  ...
  const handleRemoveEntrance = (id: string) => {
    ...
    // Unassign any checkers assigned to this entrance
    setAssignments((prev) => {
      const next = { ...prev };
  ...
  // Get checkers assigned to a specific entrance
  const getCheckersForEntrance = (entranceId: string): Checker[] => {
    return checkers.filter((c) => assignments[c.id] === entranceId);
  };
